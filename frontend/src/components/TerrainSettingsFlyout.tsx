    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, opacity: controlsDisabled ? 0.4 : 1, pointerEvents: controlsDisabled ? 'none' : 'auto' }}>
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          ...
          <span style={{ color: '#5a5a7a', fontSize: 10 }}>{blobVals.smooth}</span>
        </div>
        <input type="range" min={0} max={5} step={1} value={blobVals.smooth}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          <span>Waviness</span>
          <span style={{ color: '#5a5a7a', fontSize: 10 }}>{Math.round(blobVals.bump * 100)}%</span>
        </div>
        <input type="range" min={0} max={60} step={1} value={Math.round(blobVals.bump * 100)}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          <span>Inset</span>
          <span style={{ color: '#5a5a7a', fontSize: 10 }}>{blobVals.offset > 0 ? '+' : ''}{Math.round(blobVals.offset * 100)}%</span>
        </div>
        <input type="range" min={-80} max={30} step={1} value={Math.round(blobVals.offset * 100)}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          ...
          <span style={{ color: '#5a5a7a', fontSize: 10 }}>{blobVals.sweepFreq.toFixed(2)}</span>
        </div>
        <input type="range" min={40} max={100} step={1} value={Math.round(blobVals.sweepFreq * 100)}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          ...
          <span style={{ color: '#5a5a7a', fontSize: 10 }}>{blobVals.lobeFreq.toFixed(1)}</span>
        </div>
        <input type="range" min={20} max={50} step={1} value={Math.round(blobVals.lobeFreq * 10)}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          ...
          <span style={{ color: '#5a5a7a', fontSize: 10 }}>{Math.round(blobVals.lobeAmp * 100)}%</span>
        </div>
        <input type="range" min={0} max={100} step={1} value={Math.round(blobVals.lobeAmp * 100)}
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
          ...
          <span style={{ color: '#5a5a7a', fontSize: 10 }}>{Math.round(blobVals.lobeThreshold * 100)}%</span>
        </div>
        <input type="range" min={0} max={40} step={1} value={Math.round(blobVals.lobeThreshold * 100)}
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button style={modeBtn(blobVals.lobeDirection >= 0)} onClick={() => setBlob('lobeDirection', 1)}>Outward</button>
          <button style={modeBtn(blobVals.lobeDirection < 0)} onClick={() => setBlob('lobeDirection', -1)}>Inward</button>
        </div>
      </div>
    </div>
  )
